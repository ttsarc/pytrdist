...
from seminars.choices import TYPE_CHOICE
...
def edit_index(request):
    ...
    seminars = Seminar.objects.all().filter(
        ...
        status__in=[0, 1]
    ).annotate(entry_count=Count('seminarentryuser'))
    return render_to_response(
...
def detail(request, seminar_id):
    ...
    if request.user.is_authenticated() and (
        SeminarEntryUser.objects.is_entered(seminar, request.user)
    ):
        is_entered = True
...
def _add_entry_user(seminar, user):
    dl_user_obj, created = SeminarEntryUser.objects.get_or_create(
...
def entry(request, seminar_id):
    ...
    if seminar.entry_status == 0:
        messages.add_message(
            ...
            '申し訳ございません。定員数を超えてしまったため申込できません。')
...
def my_entry_history(request):
    ...
    """
    try:
        histories = SeminarEntryUser.objects.filter(
            user=request.user
        ).order_by('-add_date')
