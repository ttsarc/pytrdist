...
from accounts.choices import PREFECTURES_CHOICE
...
class Seminar(models.Model):
    ...
    user = models.ForeignKey(
        ...
        verbose_name='作成ユーザー', null=True, on_delete=models.SET_NULL)
    company = models.ForeignKey(
        Company, verbose_name='掲載企業')
    ...
    target = models.TextField(
        '対象企業', max_length=200, blank=True,
        help_text='ターゲットとする方がどんな方かご記入ください')
    detail = models.TextField(
        '詳細説明文', max_length=5000,
        help_text='セミナーの詳細をご記入ください')
    ...
    exhibition_time = models.CharField(
        '開催時間', max_length=50,
        help_text='例：18:00 - 20:00')
    accepting_start = models.CharField(
        '受付開始時間', max_length=50,
        help_text='例：17:30受付開始')
    promoter = models.CharField(
        '主催者', max_length=50,
        help_text='例：株式会社ミロク情報サービス')
    ...
    place_name = models.CharField(
        '会場名', max_length=100,
        help_text='例：新宿センタービル')
    prefecture = models.SmallIntegerField(
        '都道府県', choices=PREFECTURES_CHOICE)
    address = models.CharField(
        '会場住所', max_length=255)
    place_url = models.URLField(
        '会場に関するURL', blank=True,
        help_text='施設のサイトURL、GoogleマップのURL等')
    limit_number = models.IntegerField(
        '申し込み上限数',
        help_text='半角数字でご入力ください。システムで利用する数字です。\
        申し込み数がこの数に達すると満員となり、申し込みができなくなります。')
    limit_datetime = models.DateTimeField(
        '申し込み終了時間', blank=True,
        help_text='この時刻を過ぎると申し込みができなくなります。')
    mail_title = models.CharField(
        '自動返信メールタイトル', max_length=50,
        help_text='例：マーケティングセミナー受付完了')
    mail_text = models.TextField(
        '自動返信メール本文', max_length=2000,
        help_text='申し込み完了メールに記載される文章です。詳細、連絡先などを必ずご記入ください。')
    entry_status = models.SmallIntegerField(
        '受付状態', choices=ENTRY_STATUS_CHOICE, default=1,
        help_text='満員になると自動で受付終了になります。再開する場合は手動でご変更ください。')
    notable_rank = models.SmallIntegerField(
        '注目ランク', default=0,
        help_text='注目用。0で非表示、大きい方が上に出ます。0～32767')
    status = models.SmallIntegerField(
        '公開状態', choices=STATUS_CHOICE, default=0)
    ...
    def is_open(self):
        if self.limit_datetime < datetime.datetime.utcnow().replace(
            tzinfo=utc
        ):
            return False
        else:
...
class SeminarEntryUser(models.Model):
    ...
    user = models.ForeignKey(
        settings.AUTH_USER_MODEL,
        verbose_name='参加したユーザー', db_index=True)
    add_date = models.DateTimeField(
        '申し込み日', auto_now_add=True)
...
class SeminarEntryLog(models.Model):
    ...
    # Seminar
    seminar_id = models.IntegerField(
        verbose_name='セミナーID', null=True, blank=True)
    ...
    user_id = models.IntegerField(
        verbose_name='ユーザーID', null=True, blank=True)
    email = models.CharField(
        'メールアドレス', max_length=255)
    # MyUserProfile
