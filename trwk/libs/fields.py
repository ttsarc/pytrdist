...
class ContentTypeRestrictedFileField(FileField):
    ...
    Same as FileField, but you can specify:
        ...
        * max_upload_size
            - a number indicating the maximum file size
              allowed for upload.
            2.5MB - 2621440
            5MB - 5242880
            10MB - 10485760
            ...
            if content_type in self.content_types:
                if file._size > self.max_upload_size:
                    raise forms.ValidationError(
                        'ファイルサイズは %s 以下でお願い致します。 現在のファイルサイズ %s'
                        %
                        (
...
def generate_thumb(img, thumb_size, format):
    """
    Generates a thumbnail image and
...
class ImageWithThumbsField(ImageField):
    ...
    With the default storage backend if photo.jpg already exists
    it will use these filenames:
    photo_.jpg
    photo_.125x125.jpg
    photo_.300x200.jpg

    Note: django-thumbs assumes that if filename
    "any_filename.jpg" is available
    filenames with this format "any_filename.[widht]x[height].jpg"
...
def confirmation_field():
    ...
    text = (
        """必ず<a href="/pages/user-policy/" target="_blank" class="policy">"""
        """<strong>利用規約</strong>"""
        """</a>をよくお読みになり、同意の上チェックしてください。""")
    return django_forms.BooleanField(
        label="利用規約に同意する",
        help_text=mark_safe(text))
