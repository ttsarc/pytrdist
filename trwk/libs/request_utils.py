...
from django.utils.timezone import make_naive, utc, get_default_timezone
...
def set_recent_checked(request, model, pk):
    time = make_naive(
        datetime.utcnow().replace(tzinfo=utc), get_default_timezone()
    ).strftime('%s%f')
    checked_item = {time: {
        'model': model,
        'pk':  int(pk),
    ...
    if (
        'checked_items' not in request.session or
        not isinstance(request.session['checked_items'], dict)
    ):
