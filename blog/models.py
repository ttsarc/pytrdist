...
from blog.choices import STATUS_CHOICE
...
class PostImage(models.Model):
    ...
    title = models.CharField(
        'タイトル',
        max_length=255,
        help_text="管理画面でのラベル、altなどに使われます")
...
class Post(models.Model):
    ...
    slug = models.SlugField(
        'スラッグ',
        unique=True,
        help_text='URLに使われます。URLの維持とSEOのため手入力してください。半角英数とハイフン')
    content = models.TextField(
        '本文',
        help_text='画像にアップしたものを  &lt;!-- img {画像ID} --&gt;  で埋め込むことができます。')
    category = models.ManyToManyField(
        PostCategory, blank=True, verbose_name='カテゴリー')
    main_img = models.ForeignKey(
        PostImage, blank=True, null=True,
        verbose_name='メイン画像', related_name="post_with_main_img")
    img = models.ManyToManyField(
        PostImage, blank=True, verbose_name='画像')
    author = models.ForeignKey(
        settings.AUTH_USER_MODEL, verbose_name='著者',
        null=True, on_delete=models.SET_NULL)
    update_date = models.DateTimeField(
        '更新日', default=timezone.now, blank=True)
    add_date = models.DateTimeField(
        '登録日', auto_now_add=True)
    status = models.SmallIntegerField(
        '公開状態', choices=STATUS_CHOICE, default=1)
    ...
    def content_with_img(self, size=None):
        ...
        if not embed_tags:
            return content
        try:
            embed_imgs = PostImage.objects.filter(
                pk__in=[int(img[1]) for img in embed_tags]
            )
        except:
            import sys
        ...
        for img in embed_tags:
            try:
                ...
                img_tag = (
                    """<a href="%s" target="_blank" class="post-thumbnail">"""
